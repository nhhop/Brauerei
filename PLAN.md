...
- **Aktor-Intervallbetrieb / Duty-Cycle** (vorgemerkt 2026-08-12) — Aktoren, die im „Ein"-Zustand nicht dauerhaft, sondern in Intervallen laufen sollen, z.B. Rührwerk im Gärbehälter. UI-Vorbild BrewTools: ein Slider zwischen 0/60 (aus) und 60/0 (dauerhaft an); Zwischenwerte wie 25/35 geben Minuten an/aus pro Stunde an. Technisch vermutlich ein Wrapper/Decorator um einen bestehenden Aktor, der per `tick()` selbst zwischen an/aus umschaltet — offen, ob als generische `SensActCtrl`-Abstraktion (dann für alle Consumer nutzbar, analog `DualStageController`) oder BrewControl-lokal wie `ProgramRunner`.
