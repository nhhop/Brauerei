...
- Details: [SESSION.md](SESSION.md), Architektur-Referenz in [SensActCtrl/README.md](SensActCtrl/README.md)
...
- Details: [SESSION.md](SESSION.md), Architektur-/API-Referenz in [BrewControl/README.md](BrewControl/README.md)
...
- ~~**Kabellose SensActCtrl-Knoten über die Web-UI**~~ ✓ — abgeschlossen 2026-08-21: `type:"Remote"` in `DynamicItems`/`AddItemModal`, der Reihe nach für alle drei Transporte (MQTT, Webhook, ESP-NOW) umgesetzt, inkl. des `EspNowTransport::initEspNow_()`-Fixes (verbindet ESP-NOW nicht mehr auf Kosten von BrewControls eigenem WLAN). Zwei-Geräte-Leaf-Beweis (echter State/Meta-Empfang von einem tatsächlichen zweiten Board) für alle drei Transporte nachgezogen bis 2026-08-31. Details: [SESSION.md](SESSION.md) 2026-08-21/29/31.
...
- **`pio device monitor` auf TinyUSB-CDC unter Windows stabilisieren** (ESP32-S2 und -S3 betroffen): Monitor verliert sporadisch Output. Workaround: PowerShell-Skript mit `System.IO.Ports.SerialPort`, DTR/RTS-Toggle für Reset, `ReadExisting()`-Loop (siehe `BrewControl/README.md`). Mögliche Permanent-Fixes: `--filter direct`, anderes Terminal (PuTTY/Tera Term), Monitor-Reconnect-Tuning.
...
- **LittleFS-Boards (esp32dev/lolin_s2_mini) ohne Boot-Flash-Recovery + Log-Retention** (2026-08-28) — `FirmwareUpdater::flashFromSdImage()` passt nicht mehr in die 256-KB-Datenpartition (Netzwerk-OTA unberührt); `LogStore` hat dort keine Rotation, sollte nicht unbegrenzt loggen.
- **GPIO/LEDC-Leak-Fix (2026-08-14) ohne Hardware-Nachtest** — der Fix (`ledcDetachPin()` in `end()`, `removeActuator`/`removeSensor` rufen jetzt `end()`) ist umgesetzt, der praktische Nachtest ("Pin nach Löschen erneut mit Sensor belegen, ohne Reboot") blieb mangels Board offen.
- **ESP-NOW verwirft Pakete >250 Byte silently.** `EspNowTransport::sendDataPacket_()` gibt bei Überschreitung `false` zurück, kein Retry. Betrifft v.a. Controller mit vielen Params (DualStage/SplitRangePID) — deren Meta-Publish kann dauerhaft scheitern. Seit 2026-08-29 zeigt `lastErrorMessage()` immerhin "Paket zu groß (…)" an, aber nichts liest das aktiv aus/loggt es.
...
- **`POST /api/files/upload` prüft `File::write()`-Rückgabe nicht** (2026-09-01) — bei einem Short-Write auf die SD-Karte wird die Datei still abgeschnitten, der Handler meldet trotzdem `200 ok`. Beim SD-Boot-Flash-Test einmal reproduziert (1,32 MB → 618 KB auf der SD gelandet); Re-Upload lief dann fehlerfrei durch. `flashFromSdImage()` fängt das truncated Image ab (Update.end schlägt fehl, Board bootet weiter), aber der Upload sollte den Fehler selbst melden statt `ok` zu senden.
...
Roadmap und Session-Log leben ausschließlich hier auf Root-Ebene
(`PLAN.md`/`SESSION.md`/`SESSION-archive.md`), unabhängig davon, welches
Teilprojekt eine Änderung betrifft. Grundsätzliche, fürs Verständnis eines
Teilprojekts nötige Architektur-/API-Referenz steht in dessen `README.md`
([SensActCtrl/README.md](SensActCtrl/README.md),
[BrewControl/README.md](BrewControl/README.md)) — SensActCtrl behält seine
README bewusst eigenständig verständlich, da die Library separat als
PlatformIO-/Arduino-Paket veröffentlichbar bleibt.
