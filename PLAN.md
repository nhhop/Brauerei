...
- **MQTT-Einstellungen (2026-08-19):** `/settings/mqtt` — externer Broker (Host/Port/Creds/TLS) oder embedded Broker (`hsaturn/TinyMqtt`, Build-Zeit-Auth-Patch); `MqttService` verdrahtet `SettingsStore`+`DynamicItems`, Sensoren/Aktoren/Regler werden live nachgeführt. Alle 3 Boards unterstützt (`BREWCTL_HAS_EMBEDDED_MQTT_BROKER`), Flash-Kosten real gemessen: +10 KB kombiniert.
