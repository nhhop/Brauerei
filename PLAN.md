...
- **Kabellose SensActCtrl-Knoten über die Web-UI** (vorgemerkt 2026-08-21) — `RemoteSensor`/`RemoteActuator`/`RemotePublisher` existieren in der Library vollständig und funktionieren über jeden `ITransport` (MQTT, ESP-NOW, Webhook), sind aber nirgends an `DynamicItems`/`AddItemModal` angebunden — nutzbar bisher nur programmatisch in einem eigenen Sketch. Anders als die generischen MQTT-Sensor/Aktor-Typen (beliebiger Topic, für Fremdgeräte) geht es hier um echte SensActCtrl-Node-zu-Node-Kommunikation (ein zweiter ESP32 mit eigenen Sensoren/Aktoren, der sich per Meta/State-Protokoll bei BrewControl anmeldet). Größerer Scope als die beiden Punkte oben: braucht auch eine Pairing-/Discovery-UX für ESP-NOW (wie findet/verbindet man im Web-UI einen zweiten, noch unbekannten ESP32?) — eigene Spec vor Implementierung sinnvoll.
