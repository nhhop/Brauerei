...
- **`POST /api/files/upload` prüft `File::write()`-Rückgabe nicht** (2026-09-01) — bei einem Short-Write auf die SD-Karte wird die Datei still abgeschnitten, der Handler meldet trotzdem `200 ok`. Beim SD-Boot-Flash-Test einmal reproduziert (1,32 MB → 618 KB auf der SD); Re-Upload lief dann sauber. `flashFromSdImage()` fängt das truncated Image ab (Update.end schlägt fehl, Board bootet weiter), aber der Upload sollte den Fehler selbst melden statt `ok` zu senden.
- **`web/src/types.ts` weicht an drei Stellen von der Firmware ab** (2026-09-01, beim OpenAPI-Abgleich gefunden) — `ProgramConfig` fehlen `stepStartedEpoch` und `elapsedAtPauseSec` (werden von `ProgramRunner::serialize()` immer geliefert); `DashboardConfig.charts`/`.programs` sind optional deklariert, `DashboardStore::serialize()` emittiert sie aber immer; `AppSettings.firmware/time/mqtt/webhook/espnow` sind optional deklariert, `SettingsStore::serialize()` liefert immer alle sechs Sektionen. Nur Typen zu eng/zu lose — kein Laufzeitfehler, aber die Regel „`types.ts` spiegelt die Wire-Form" ist verletzt.
...
- **`GET /api/settings` liefert `mqtt.password` im Klartext** (2026-09-01) — bei fehlender Authentifizierung auf dem gesamten API. Hängt am Backlog-Punkt „Zugriffsschutz / Auth"; separat wäre ein Write-only-Feld (Rückgabe maskiert, leerer String = unverändert) die kleinere Lösung.
- **`GET /api/snapshot` liefert bei Puffer-Überlauf einen leeren `200`-Body** (2026-09-01) — `serializeRegistry()` gibt 0 zurück, wenn die Registry `kSnapshotCap` (4160 Byte) sprengt; der Handler prüft das nicht und sendet trotzdem `200`. Sollte `500`/`503` sein, sonst sieht das Frontend eine leere Registry statt eines Fehlers.
